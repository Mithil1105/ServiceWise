    ...
    <div className="space-y-6 animate-fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="text-muted-foreground">Fleet overview and alerts</p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <div className="flex items-center justify-center gap-1">
                  <p className="text-2xl font-bold">{snapshot.kmDriven.toLocaleString()}</p>
                  {snapshot.kmWarning && (
                    <Tooltip>
              ...
              <div className="text-center">
                <p className="text-2xl font-bold">{snapshot.servicesCompleted}</p>
                <p className="text-sm text-muted-foreground">Services Completed</p>
              </div>
              <div className="text-center">
                <p className={`text-2xl font-bold ${snapshot.criticalAlerts > 0 ? 'text-destructive' : ''}`}>
                  {snapshot.criticalAlerts}
                </p>
                <p className="text-sm text-muted-foreground">Critical Alerts</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold">₹{snapshot.expenses.toLocaleString()}</p>
        ...
        <CardContent>
          {attentionLoading ? (
            ...
            <div className="text-center py-6 text-muted-foreground">
              ...
              <p>All caught up! No immediate attention needed.</p>
            </div>
          )}
        </CardContent>
      </Card>
        ...
        <Card className="stat-card">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{criticalServices?.length || 0}</p>
            </div>
          </div>
        </Card>
      </div>
        ...
        <CardContent>
          {criticalLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              <Wrench className="h-8 w-8 mx-auto mb-2 opacity-50" />
              <p>No critical services pending</p>
            </div>
          )}
        </CardContent>
      </Card>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {criticalServices?.slice(0, 5).map((item) => (
              ...
              >
                ...
                <span className={`text-sm font-medium ${item.status === 'overdue' ? 'text-destructive' : 'text-warning'}`}>
                  {item.status === 'overdue'
                    ? `Overdue by ${Math.abs(item.remaining_km).toLocaleString()} km`
                    : `Due in ${item.remaining_km.toLocaleString()} km`}
                </span>
